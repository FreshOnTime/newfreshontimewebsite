import { z } from "zod";
import { normalizeFeaturedImage, parseRecipeContent, recipeContentSchema } from "@/lib/recipeContent";

export const recipeAdminInputSchema = z.object({
...
export const recipeAdminPatchSchema = recipeAdminInputSchema.partial();

export function serializeRecipeForAdmin(row: {
  id: string;
  title: string;
  slug: string;
  excerpt: string;
  ...
  return {
    ...
    content: parseRecipeContent(row.content),
