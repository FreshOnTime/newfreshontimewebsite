...
const shopLinks = [
  ...
  { name: "Meals on Deals", href: "/meals" },
      ...
      <div className="relative border-b border-white/5">
        <div className="container mx-auto flex flex-col items-center justify-between gap-8 px-6 py-16 md:flex-row md:px-12">
          <div className="max-w-lg text-center md:text-left">
            ...
            <p className="font-light text-zinc-400">Seasonal picks, new makers, partner stories, and useful FreshPick updates.</p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-12 md:grid-cols-12 lg:gap-20">
          <div className="space-y-7 md:col-span-4">
            ...
            <p className="max-w-sm font-light leading-relaxed text-zinc-400">
              Fresh groceries, local food discoveries, recurring delivery, and curated partnerships with the people behind the food.
            </p>
              ...
              <div className="flex gap-3 pt-2">
                {socialLinks.map((item) => (
                  <a
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-full border border-white/5 bg-white/[0.035] text-zinc-400 transition-all duration-300 hover:border-emerald-300/30 hover:bg-emerald-300/10 hover:text-emerald-300"
                  >
          ...
          <div className="grid grid-cols-2 gap-10 md:col-span-8 md:grid-cols-3">
            <div>
              <h4 className="mb-7 font-serif text-lg italic text-white">Shop</h4>
              <ul className="space-y-4">
                {shopLinks.map((item) => (
                  <li key={item.name}>
                    <Link href={item.href} className="text-sm font-light tracking-wide text-zinc-500 transition-colors hover:text-emerald-300">
                      {item.name}
                    </Link>
                  </li>
                ))}
