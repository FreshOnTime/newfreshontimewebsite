    ...
    <main className="min-h-screen bg-[#f4f5f1] lg:grid lg:grid-cols-[1.08fr_0.92fr]">
      <section className="relative hidden min-h-screen overflow-hidden bg-[#08130d] lg:block">
        <Image
          ...
          alt="Fresh food selected by FreshPick"
          fill
          priority
          sizes="55vw"
        ...
        <div className="absolute inset-x-0 bottom-0 p-10 xl:p-16">
          <div className="max-w-2xl text-white">
            <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/10 px-4 py-2 text-[9px] font-bold uppercase tracking-[0.24em] text-emerald-100 backdrop-blur-md">
              <Sparkles className="h-3.5 w-3.5" /> Your FreshPick
            </span>
            <h2 className="mt-7 font-serif text-5xl font-normal leading-[0.94] tracking-[-0.035em] xl:text-7xl">
              Pick up where your <span className="italic text-emerald-200">taste left off.</span>
            </h2>
            <p className="mt-6 max-w-xl text-base font-light leading-8 text-white/68">
              Sign in for saved bags, repeat reminders, personal recommendations and a simpler way back to the food you actually buy.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="flex min-h-screen items-center justify-center px-5 py-24 sm:px-8 lg:px-12 xl:px-16">
        <div className="w-full max-w-[460px]">
          ...
          <form onSubmit={handleSubmit} className="mt-9 space-y-5">
            <div className="space-y-2">
              ...
              <Input
                ...
                required
                className="h-13 rounded-[1rem] border-zinc-200 bg-white px-4 shadow-none focus-visible:ring-emerald-700/20"
              />
            </div>
            ...
            <Button
              type="submit"
              className="h-13 w-full rounded-full bg-zinc-950 text-[10px] font-bold uppercase tracking-[0.18em] text-white shadow-none transition-colors hover:bg-emerald-950"
            ...
            <div className="flex items-center gap-4 py-1">
              ...
              <span className="text-[9px] font-semibold uppercase tracking-[0.16em] text-zinc-400">or</span>
              <span className="h-px flex-1 bg-zinc-200" />
            </div>
            ...
            <Button
              type="button"
              variant="outline"
              className="h-13 w-full rounded-full border-zinc-300 bg-white text-sm font-medium text-zinc-700 shadow-none hover:bg-white"
              disabled={isLoading}
          ...
          <div className="mt-8 rounded-[1.25rem] border border-zinc-200 bg-white/70 p-4">
            <div className="flex gap-3">
              <LockKeyhole className="mt-0.5 h-4 w-4 shrink-0 text-emerald-800" />
              <p className="text-xs font-light leading-5 text-zinc-500">Your account keeps your orders, bags and preferences together. FreshPick does not create a personal profile until you actually use the service.</p>
            </div>
          </div>

          <p className="mt-8 text-sm text-zinc-500">
            New to FreshPick?{' '}
            <Link href="/auth/signup" className="font-semibold text-emerald-800 hover:text-emerald-950">Create an account</Link>
          </p>
        </div>
      </section>
    </main>
